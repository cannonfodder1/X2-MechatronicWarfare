class X2Effect_FusionReactor extends X2Effect_Persistent;
...
function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	...
	EventMgr.RegisterForEvent(EffectObj, 'AbilityActivated', class'XComGameState_Effect_FusionReactor'.static.AbilityActivatedListener, ELD_OnStateSubmitted);
}
/*
function int GetAttackingDamageModifier(XComGameState_Effect EffectState, XComGameState_Unit Attacker, Damageable TargetDamageable, XComGameState_Ability AbilityState, const out EffectAppliedData AppliedData, const int CurrentDamage, optional XComGameState NewGameState)
{
	local XComGameState_Effect_FusionReactor FieldEffectState;
	local XComGameState_Item SourceWeapon;
	local int BonusDamage;

	FieldEffectState = XComGameState_Effect_FusionReactor(EffectState);
	if (FieldEffectState != none)
	{
		...
		if (SourceWeapon != none && SourceWeapon.InventorySlot == eInvSlot_PrimaryWeapon)
		{
			...
			{
				FieldEffectState = XComGameState_Effect_FusionReactor(NewGameState.ModifyStateObject(FieldEffectState.Class, FieldEffectState.ObjectID));
	...
	{
		`RedScreen("X2Effect_FusionReactor was given an EffectState" @ EffectState.ToString() @ "which is not the right class - should be XComGameState_Effect_FusionReactor! @gameplay @jbouscher");
	}
...
{
	if (XComGameState_Effect_FusionReactor(EffectState).LastEnergyExpendedContext == AbilityContext)
	{
...
simulated function AddX2ActionsForVisualization_Sync( XComGameState VisualizeGameState, out VisualizationActionMetadata ActionMetadata )
{
	...
	local XComGameState_Effect_FusionReactor EffectState;
	...
	{
		EffectState = XComGameState_Effect_FusionReactor(XComGameState_Unit(ActionMetadata.StateObject_NewState).GetUnitAffectedByEffectState(EffectName));
		if (EffectState != none && EffectState.EnergyAbsorbed > 0)
		{
			...
			PlayAnimation.Params.AnimName = 'ADD_FusionReactor';
...
function name TargetAdditiveAnimOnApplyWeaponDamage(XComGameStateContext Context, XComGameState_Unit TargetUnit, XComGameState_Effect EffectState)
{
	...
	{
		if (class'XComGameState_Effect_FusionReactor'.default.AbsorbedAbilities.Find(XComGameStateContext_Ability(Context).InputContext.AbilityTemplateName) != INDEX_NONE)
			return 'ADD_FusionReactor';
	}
...
DefaultProperties
{
	EffectName = "FusionReactor"
	DuplicateResponse = eDupe_Ignore
	GameStateEffectClass = class'XComGameState_Effect_FusionReactor';
