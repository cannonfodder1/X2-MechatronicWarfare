class X2TargetingMethod_Supernova extends X2TargetingMethod_PathTarget;
...
function Init(AvailableAction InAction, int NewTargetIndex)
{
	...
	RadiusTiles = Sqrt(class'X2Ability_SparkAbilitySet_MW'.default.SUPERNOVA_RADIUS_SQ);
	...
	if( TargetLocations.Length > 0 )
	{
		...
		foreach History.IterateByClassType(class'XComGameState_Unit', CheckUnitState)
		{
			if( CheckUnitState.ObjectID != UnitState.ObjectID &&
				CheckUnitState.GetTeam() == eTeam_Alien &&
				!CheckUnitState.IsDead() &&
				!CheckUnitState.bRemovedFromPlay)
			{
				// Check to see if this Unit is within the Targeting Area
